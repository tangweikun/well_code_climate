...
import { Spin, Divider, Table, Modal, Form, Input, Select, TimePicker, Button, message } from 'antd';
      ...
      onOk={() => {
        ...
        form.validateFields().then(async (values) => {
          const query = {
            ...
            workdprice: _get(values, 'workdprice') || _get(currentRecord, 'workdprice'),
            weekdprice: _get(values, 'weekdprice') || _get(currentRecord, 'weekdprice'),
          ...
          if (RULE_LIST.some((x: any) => !_get(x, 'price'))) {
          ...
          if (_get(unreasonablePriceArr, 'length', 0) > 0) {
      ...
      {!isLoading && _get(traincodeOptions, 'length', 0) > 0 && (
        <Spin spinning={spinning}>
          ...
          >
            ...
            <Table columns={columns} dataSource={workList} rowKey="id" pagination={false} />
            ...
            <Table columns={columns} dataSource={weekList} rowKey="id" pagination={false} />
          </Form>
        </Spin>
      )}
    </Modal>
