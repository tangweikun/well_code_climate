        ...
        onOk={() => {
          form.validateFields().then(async (values) => {
            const res = await _addExam({
              ...
              testEtc: _get(values, 'testEtc'),
              testPlace: _get(values, 'testPlace'),
              idNumber: idCard,
        ...
        <Form form={form} autoComplete="off" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
          ...
          {studentData && (
            <Row style={{ width: 320, marginLeft: 75 }}>
              {studentInfo.map((x: any, index: any) => {
                return (
                  <Col key={index} span={12} style={{ marginBottom: 20 }}>
                    {x.label}:{x.value}
                  </Col>
                );
              })}
          ...
          <Form.Item
            label="考试场次"
            name="testEtc"
            rules={[{ whitespace: true, required: true, message: '请输入考试场次' }, RULES.EXAM_TIME]}
          >
            <Input />
          </Form.Item>
          <Form.Item
            label="考试场地"
            name="testPlace"
            rules={[{ whitespace: true, required: true, message: '请输入考试场地' }, RULES.EXAM_PLACE]}
          >
            <Input />
          </Form.Item>
        </Form>
