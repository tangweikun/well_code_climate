...
export default function ExamCompareTable(props: IProps) {
  ...
  const { isLoading } = useFetch({
    ...
    callback: (data: any) => {
      ...
      subject3Arr.map((item: any, index: any) => {
  ...
  return (
    <div>
      <div className="mb20">统计对比年份：{year + ' : ' + (Number(year) - 1)}</div>
      {isLoading && <Loading />}
      {!isLoading && (
        <Table columns={columns} loading={isLoading} bordered dataSource={data} rowKey="month" pagination={false} />
      )}
    </div>
  );
}
