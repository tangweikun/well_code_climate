...
export default function VersionRecord(props: any) {
  ...
  return (
    <Modal visible width={800} title={'版本记录'} maskClosable={false} onCancel={onCancel} footer={null}>
      <div style={{ marginBottom: 20 }}>{_get(data, 'title')}</div>

      <Button type="primary" onClick={_switchReleaseVisible} style={{ marginBottom: 20 }}>
        发布新版本
      </Button>

      {_get(currentRecord, 'type') === '1' && (
        <Alert message="当车辆未添加特定围栏版本，默认使用驾校通用围栏" type="warning" style={{ marginBottom: 20 }} />
      )}
      ...
      <Table
        ...
        loading={isLoading}
        bordered
        dataSource={_get(data, 'versionRecode', [])}
        rowKey={(record) => _get(record, 'fid')}
      />
    </Modal>
  );
}
