...
export async function _getCoachCardList(query: {
  page: number;
  limit: number;
  barcode?: string; // 卡条形码
...
export async function _addCard(
  ...
  return await request(`${NOT_CORE_PREFIX}/v1/coa/saveCoachCard`, 'POST', query, {
    withFeedback: true,
    customHeader: { menuId: 'coachCard', elementId: elementId },
  });
}
...
export async function _application(query: {
  applystatus?: string;
  cardTypeEnum: string;
  cid: string;
  remarks?: string;
...
//保存指纹采集模板
export async function _saveCoaFingerTemplate(query: {
