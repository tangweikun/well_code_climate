...
export default function Details(props: any) {
  ...
  return (
    <Modal visible width={800} title={'安全员信息详情'} maskClosable={false} onCancel={onCancel} footer={null}>
      <Form form={form} autoComplete="off" labelCol={{ span: 10 }} wrapperCol={{ span: 14 }}>
        ...
        <Row>
          ...
          <ItemCol span={8} label="备案状态">
            {registeredExamFlagHash[_get(data, 'coaCoachExtinfoEntity.registeredSafeflag', 0)]}
          </ItemCol>
          {_get(data, 'coaCoachExtinfoEntity.registeredSafeflag', 0) === '3' && (
            <ItemCol span={8} label="原因">
              {_get(data, 'coaCoachExtinfoEntity.messageSafe', '')}
            </ItemCol>
          )}
        </Row>
        ...
        <Row>
          ...
          <ItemCol span={8} label="供职状态">
            {employStatusHash[_get(data, 'employstatusAqy', '00')]}
          </ItemCol>
