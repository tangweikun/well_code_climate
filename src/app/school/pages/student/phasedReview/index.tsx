    ...
    },
    {
      ...
      render: (_: void, record: any) => (
        <div>
          ...
          {/* 审核拒绝的报审记录单，因培训记录单不合格，要求可以单独上传培训记录单 uploadStagePdfFlag 1-表示要求显示上报培训记录单到监管按钮 */}
          <AuthButton
            loading={_get(currentRecord, 'said') === _get(record, 'said') && getSupervisorPlatformLoading}
            authId="student/phasedReview:btn10"
            insertWhen={Number(_get(record, 'uploadStagePdfFlag', 0)) === 1}
            className="operation-button"
            onClick={() => {
              setCurrentRecord(record);
              getSupervisorPlatformRun({ id: record.said });
            }}
          >
            补传培训记录
          </AuthButton>
