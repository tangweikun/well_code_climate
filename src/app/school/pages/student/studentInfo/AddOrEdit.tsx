...
import { readIdCardData, getIdCardId } from 'utils';
...
export default function AddOrEdit(props: any) {
  ...
  const [stuType, setStuType] = useState(isEdit ? _get(currentRecord, 'studenttype', '') : studenttype);
  ...
  // 银行驾校账户
  const { data: openAccountData = [] } = useFetch({
    request: _getOpenAccount,
  });

  // 班级数据
  // FIXME:wy
  const { data: classList = [] } = useFetch<any>({
  ...
  // FIXME:wy
  const { data: cardMoney, res } = useFetch<any>({
      ...
      >
        {
          <Button
            ...
            onClick={async () => {
              ...
              const physicalIdCardRes = await getIdCardId();
              setIccardcode(_get(physicalIdCardRes, 'cardNo', ''));
            }}
          >
            读二代证
          </Button>
                ...
                <ItemCol span={8} label="业务类型" name="busitype" rules={[{ whitespace: true, required: true }]}>
                  <Select
                    ...
                    onChange={(value: string) => {
                      ...
                      const currentStudentType = isEdit ? _get(currentRecord, 'studenttype', '') : studenttype;
              ...
              <Row>
                ...
                {isShowClassCoach && (
                  <>
                    ...
                    {train_price_online && (
                      <ItemCol required span={8} label="银行账户" rules={[{ required: true }]}>
                        <Select
                          value={bankchannelid}
                          onChange={(value: any) => {
                            const openAccountDataInfo: any = openAccountData.filter(
                              (x: any) => x.bankchannelid === value,
                            );
                            setBankchannelid(_get(openAccountDataInfo, '0.bankchannelid', ''));
                            setBankaccount(_get(openAccountDataInfo, '0.bankaccount', ''));
                          }}
                        >
                          {openAccountData.map((item: any) => {
                            return (
                              <Option key={item.bankchannelid} value={item.bankchannelid}>
                                {item.bankName}
                              </Option>
                            );
                          })}
